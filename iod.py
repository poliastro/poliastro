...
def lambert(k, r0, rf, dm, tof):
    ...
    r : array
        Final position (km).
    dm : string
        Direction of motion: 'short' or 'long'.
    tof : float
        Time of flight (s).
    ...
    assert r0.shape == (3,)
    assert rf.shape == (3,)
    if dm not in ('short', 'long'):
        raise ValueError("Direction of motion must be either "
                         "'short' or 'long', received {}".format(dm))
    va, vb, error = _astiod.lambertuniv(r0, rf, dm[0].upper(), 'N', tof, k)
