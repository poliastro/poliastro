...
def plot_orbits(mu, targeted_po_orbits, colourby, cb_label = 'JC', colourmap = 'plasma',title = None, data_trace=None, save=False):
    """
    ...
    norm = matplotlib.colors.Normalize(vmin=colour_range[0],
                                        vmax=colour_range[1])
    colours = []
    for i in colourby:
    ...
    for i in range(len(targeted_po_orbits)):
        fig.add_trace(go.Scatter3d(x=targeted_po_orbits[i]['states'][:,0], 
                                   y=targeted_po_orbits[i]['states'][:,1],
                      z=targeted_po_orbits[i]['states'][:,2], mode='lines',
                      hovertext=f'c={colourby[i]:.3f}',
                      line=go.scatter3d.Line(color=f'rgba{colours[i]}',
                                              width=5)))
    ...
    colorbar_trace = go.Scatter3d(x=[None],
                                  ...
                                  cmin=colour_range[0],
                                  cmax=colour_range[1],
                                  colorbar=dict(title=dict(text=cb_label,
                                                font=dict(size=18)),
                                                thickness=10,
                                                outlinewidth=0,
    ...
    fig.update_layout(title=title, title_x=0.5, 
                      font=dict(size=18),template='plotly_dark',showlegend=False)
