...
class periodic_orbit(cr3bp_model):
    ...
    def single_shooter(
        ...
        identity_temp = self.__constraints_setup_update(
    ...
    def __FX_setup_update(self, palc_args):
        ...
        if palc_args is None:
            ...
            FX[-1] = (
                np.matmul(
                    np.transpose(self.xfree - palc_args["free_var_prev"]),
                    palc_args["delta_X*_prev"],
                )
                - palc_args["delta_s"]
            )
    ...
    def __compute_DF(
        ...
        if "t" in free_vars:
            self.DF[: self.stm_row_len, -1] = dxf_dt[self.stm_row_index]
        if self.sym_period_targ == 1:
            self.DF[: self.stm_row_len, : self.stm_col_len] = (
                self.DF[: self.stm_row_len, : self.stm_col_len] - identity_temp
            )
    ...
    def global_manifold_gen(
        ...
        tf_po_state = np.linspace(
            start_time, stop_time, num_po_states, endpoint=False
        )  # Enpoint defaults to true. last value will be replaced by first to help with surface plot
        ...
        for count_po_states in range(num_po_states):
            ...
            eigvector_tfi = np.real(
                np.matmul(
                    copy.deepcopy(result_stm["stm"][:, :, -1]),
                    eigenvect_manifold,
                )
            )
