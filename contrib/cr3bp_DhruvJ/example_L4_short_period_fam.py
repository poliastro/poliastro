...
# From D. Grebow
ig = np.array([ 0.497653463566241, 0.866025403784439, 0, 0.006195078418070,-0.008234243375973, 0])
...
orb_fam_obj.npc_po_fam(free_vars, constraints,sym_period_targ=1, Nmax=10, 
                    step_size= 1e-4, num_fam_members=2, param_continue="jc", line_search=True)

'''
PALC
'''
orb_fam_obj.palc_po_fam(free_vars, constraints,sym_period_targ=1, Nmax=10, 
                    step_size= 1e-1, num_fam_members=30, line_search=True)
...
# Add L4
for i in range(5):
    data_trace.append(go.Scatter3d(x=[lib_loc[i,0]], y=[lib_loc[i,1]], z=[0], marker=dict(
                color='red',
                size=2)))
# Add Moon
