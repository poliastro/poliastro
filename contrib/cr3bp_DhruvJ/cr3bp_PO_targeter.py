...
def multi_shooter_nodes_setup_cr3bp(mu, ic, tf, n_node, node_place_opt = 'time', int_tol=1e-12):
    ...
    results_stm = prop_cr3bp(mu, ic, tf, stm_bool=0, xcross_cond=0) # Propagate I.C. till tf
    ic_node = []
    t_node = []
    ...
    if node_place_opt == 'time':
        ...
        for i in range(1, n_node):
            index = np.argmin(abs(results_stm['t']-ti[i-1])) # compute index when index from time history when time is nearly 
            ic_node.append(results_stm['states'][index,:])
            t_node.append(results_stm['t'][index]-sum(t_node)) # t_node runs one node behind as it is the time to next node, subtract sum as ti+t2+t3 = tf
        
        t_node.append(tf-sum(t_node))# Time from final node to vicinity of desired state
        ...
        for i in range(1, n_node):
            ...
            t_node.append(results_stm['t'][indices[i]]-sum(t_node))# t_node runs one node behind as it is the time to next node, subtract sum as ti+t2+t3 = tf
