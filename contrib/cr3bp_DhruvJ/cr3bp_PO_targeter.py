...
def po_single_shooter_cr3bp(
    ...
    if "t" in free_vars and sym_period_targ != 1:
        results_stm = prop_cr3bp(
            mu, initial_guess, tf_guess, tol=int_tol, xcross_cond=1
        )
        if (
            len(results_stm["tevents"][0][:]) > 1
    ...
    results_stm = prop_cr3bp(
        mu, initial_guess, tf, stm_bool=1, tol=int_tol, xcross_cond=0
    )
    ...
    stm_col_index = [
        free_vars_index[i] for i in range(len(xfree)) if free_vars_index[i] < 6
    ]
    stm_row_index = [
        constraints_index[i]
        for i in range(len(xconstraint))
    ...
    if palc_args == None:
        ...
        FX[-1] = (
            np.matmul(
                np.transpose(xfree - palc_args["free_var_prev"]),
    ...
    while np.linalg.norm(FX) > conv_tol and count <= Nmax:
        ...
        if sym_period_targ == 1:
            DF[:stm_row_len, :stm_col_len] = (
                DF[:stm_row_len, :stm_col_len] - identity_temp
            )
