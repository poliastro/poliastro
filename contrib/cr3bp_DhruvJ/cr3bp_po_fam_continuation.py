...
class periodic_orbit_fam_continuation(periodic_orbit):
    ...
    def npc_po_fam(self, free_vars, constraints, sym_period_targ=1 / 2, conv_tol=1e-12, int_tol=1e-12, Nmax=50, 
        ...
        check_logic_val, free_vars, constraints = self.npc_po_logic_check_continueparam_update(free_vars, constraints, param_continue)    
        ...
        while count_fam_member < num_fam_members and iterflag is False:
            ...
            results, iterflag = self.single_shooter(free_vars,
                    constraints,
            ...
            # Targeter converged => save data and update the setup to compute the next family member
                ...
                self.ic = copy.deepcopy(results["states"][0, :])  # To not update save data as values are passed as object reference
                self.save_targeted_po_char(results)
                count_fam_member += 1
                
                if count_fam_member < num_fam_members:
                    self.npc_po_update(results, step_size, param_continue) # Continue the param_continue with the current step size
    ...
    def palc_po_fam(self, free_vars, constraints, sym_period_targ=1 / 2, conv_tol=1e-12, int_tol=1e-12, Nmax=50, 
        ...
        retargeted_orbit, iterflag = self.single_shooter(free_vars,
                    constraints,
                    sym_period_targ=sym_period_targ,
                    conv_tol=conv_tol,
                    int_tol=int_tol,
        ...
        while count_fam_member < num_fam_members and iterflag is False:
            ...
            if iterflag is True and line_search is True:  
                # Use Line Search: Update Step size and reset recompute
                iterflag, step_size = self.line_search(results,step_size, param_continue, line_search_params,palc_args=palc_args)
                
            elif iterflag is False:
                print("# PO family member = ", count_fam_member + 1, "\n")
                self.targeted_po_fam.append(results) # tf is updated at the end of the shooter function
                self.ic = copy.deepcopy(results["states"][0, :])  # To not update save data as values are passed as object reference
                palc_args, null_vec = self.palc_null_vect_update(free_vars, results, null_vec, palc_args)# Compute null vector and palc constraint components
    ...
    def line_search(self,results,step_size, param_continue, line_search_params, palc_args=None):
        """
        ...
        if abs(step_size) < abs(line_search_params['step_size0'] * line_search_params['lower_lim_factor']):
            print("Updated step size is too small compared to given step size. Rerun with smaller step size, attenuation factor or allowable lower limit")
