...
def benchmark(
    ...
    assert limit_exp >= 0
    steps = (10 ** np.arange(0, limit_exp + limit_exp_step, limit_exp_step)).astype(
        "i8"
    )
    ...
    for idx, iterations in enumerate(steps):
        ...
        gc.collect()
        print(
            f"... finished in {times[-1]:e} seconds ({times[-1]/iterations:e} seconds per iteration)."
        )
    ...
    fig.add_trace(
        go.Scatter(
            x=steps, y=np.array(times) / steps, name="Iterative", line=dict(width=2)
        )
    )
