PIC_SATELLITE = "data:image/png;base64,"

DEFAULTS = [
    ([1], [("id", "ID"), ("name", "NAME"), ("availability", "0000-00-00T00:00:00Z/0000-00-00T00:00:00Z"),
           ("description", "DESCRIPTION")]),
    ([1, "billboard", "eyeOffset"], [("cartesian", [0, 0, 0])]),
    ([1, "billboard"], [("horizontalOrigin", "CENTER"), ("verticalOrigin", "CENTER"), ("image", PIC_SATELLITE),
                        ("scale", 1), ("show", True)]),
    ([1, "billboard", "pixelOffset"], [("cartesian2", [0, 0])]),
    ([1, "label", "fillColor"], [("rgba", [255, 255, 0, 255])]),
    ([1, "label"], [("font", "11pt Lucida Console"), ("horizontalOrigin", "LEFT"), ("outlineWidth", 2),
                    ("show", True), ("style", "FILL_AND_OUTLINE"), ("text", "Text"), ("verticalOrigin", "CENTER")]),
    ([1, "label", "pixelOffset"], [("cartesian2", [6, -4])]),
    ([1, "label", "outlineColor"], [("rgba", [2, 100, 0, 255])]),
    ([1, "path", "show"], [("interval", "0000-00-00T00:00:00Z/0000-00-00T00:00:00Z"), ("boolean", True)]),
    ([1, "path"], [("width", 1), ("resolution", 120)]),
    ([1, "path", "material", "solidColor", "color"], [("rgba", [255, 255, 0, 255])]),
]
