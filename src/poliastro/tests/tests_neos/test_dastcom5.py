...
def test_read_headers(mock_open, mock_np_fromfile):
    ...
    mock_open.assert_any_call(os.path.join(dastcom5.DBS_LOCAL_PATH, 'dast5_le.dat'), 'rb')
    mock_open.assert_any_call(os.path.join(dastcom5.DBS_LOCAL_PATH, 'dcom5_le.dat'), 'rb')
...
@mock.patch('poliastro.neos.dastcom5.zipfile')
@mock.patch('poliastro.neos.dastcom5.os.path.isdir')
def test_download_dastcom5_creates_folder(mock_isdir, mock_zipfile, mock_makedirs, mock_request):
